...
def main(execution_date, airflow_connection, airflow_variable):
    ...
    source = airflow_home_path + '/data/medalion_dag/slv_encounters/' + execution_date

    table = 'fact_encounter'
    partition_key = 'stop_time'
    schema = {
        'date': 'DATE NOT NULL',
        'encounter_id': 'VARCHAR NOT NULL PRIMARY KEY',
    ...
    exclude_keys = ['date', 'dim_patient_key', 'dim_payer_key', 'patient_pay_amount', 'length_of_stay_num']
    ...
    if is_table_exists is False:
        exclude_keys = ['patient_id', 'payer_id']
    ...
    sql = f"""
        SELECT
            ...
            source.claim_amount - source.coverage_amount AS patient_pay_amount,
            EXTRACT(DAY FROM source.stop_time - source.start_time) AS length_of_stay_num
        ...
        LEFT JOIN dim_payer
            ON (source.payer_id=dim_payer.payer_id
