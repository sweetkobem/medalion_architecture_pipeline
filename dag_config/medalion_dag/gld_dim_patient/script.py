...
def main(execution_date, airflow_connection, airflow_variable):
    ...
    source = airflow_home_path + '/data/medalion_dag/slv_patients/' + execution_date

    table = 'dim_patient'
    key_source = 'patient_id'
    key_destination = 'dim_patient_key'
    schema = {
        'patient_id': 'VARCHAR NOT NULL',
        'patient_fullname': 'VARCHAR NOT NULL',
        'birth_date': 'DATE',
        'death_date': 'DATE',
    ...
    if is_table_exists is False:
        ...
        conn.execute(f"""
        CREATE SEQUENCE IF NOT EXISTS {key_destination} START 1;
